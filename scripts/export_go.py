...
Usage: export_go.py [--n 512] [--w brain/weights/best.npz] [--out ../../fair_backgammon/fly] [--vectors 300]
...
def parse(a):
    o = dict(n=512, w=str(ROOT / "brain" / "weights" / "best.npz"),
             out="/var/home/danel/Projects/fair_backgammon/fly", vectors=300)
    i = 0
    while i < len(a):
        if a[i] in ("--n", "--vectors") and i + 1 < len(a):
            o[a[i][2:]] = int(a[i + 1]); i += 2
        elif a[i] in ("--w", "--out") and i + 1 < len(a):
            o[a[i][2:]] = a[i + 1]; i += 2
        else:
